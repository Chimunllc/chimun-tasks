...
- **Хугацааны хямдрал (2026-09-03 шинэчлэв — хуучин «2+ хоног = 20%» ХОЦРОГДСОН):** Шатлал = **`app_config['tariffs'].tiers`** (нэг эх сурвалж, доорх ТАРИФ SYNC). Кодын `RENTAL_TIERS` fallback = 30+ хоног **55%**, 7–29 хоног **40%**, 2–6 хоног **20%**. Хоног = **календарийн өдөр** (цагаар БИШ). `orderDiscountAmount` нь авто-шат ба гар хямдралын **ИХ**-ийг авна (нэмэлт БИШ). НӨАТ хасах чагт = түрээсээс −5%.
