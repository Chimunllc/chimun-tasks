...
- **Апп:** CEO-only view `booqable` ("Түрээсийн түүх"). `loadBooqable` нь 6 view-г PostgREST anon SELECT-ээр **lazy** татна (статик тул polling-д ОРООГҮЙ). `renderBooqable` 4 таб: Орлого (KPI+сар бүрийн босоо bar+төлбөрийн хэлбэр) / Бараа / Харилцагч / Ашиглалт. `state.booqable`/`bqTab`. Хүснэгт үүсээгүй бол error төлөвт SQL ачаалах заавар.
- **Дата урсгал:** `data booqable/full-export-<TODAY>/` (download_all_fresh.py, Booqable API v4) → `postgres-migration/bq_gen.js` → `bq_02..07_*.sql` (500мөр/statement batched upsert, id-аар дедуп — lines export-д ~24% давхардал бий). `bq_01_schema.sql` урьд RUN. **Дараалал: 01→02→03→04→05→06→07.**
