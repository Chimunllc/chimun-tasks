...
// удаа гараад дараа нь алга болдог байв (осол гарахад хэнд залгахаа мэдэхгүй).
// Дүрэм: openStaffCardModal нь `info` блокийг заавал рендэрлэнэ.
{
  ok(/const rows = \[\s*\n\s*scTel\('☎️ Яаралтай', m\.emergency_name, m\.emergency_phone\)/.test(src),
     'scan: ажилтны картад яаралтай холбоо бий');
  ok(/<div class="sc-body">\$\{info\}/.test(src),
     'scan: «Хувийн мэдээлэл» блок sc-body-д рендэрлэгдэнэ');
}

// 0g) SCAN — цагийн ажилтны ЖАГСААЛТ ба МӨНГӨ тусдаа эрхтэй (2026-09-16)
// Нярав өдрийн ажилтан дуудахын тулд жагсаалтыг хардаг. Хэн хэдэн төгрөг авсан,
// банкны данс, «Цалин шилжүүлэх» товч нь өөр эрх — `canSeeHourlyMoney()`.
// ⛔ `can('salary.pay')` нь тохируулаагүй үед ЗӨВШӨӨРдөг тул загварт таараагүй ШИНЭ
//    албан тушаал цалинг чимээгүй харна. `capValue(...) === true` л зөв.
{
  ok(/function canSeeHourlyMoney\(\)\s*\{[^}]*capValue\('salary\.pay'\) === true/.test(src),
     'scan: canSeeHourlyMoney нь capValue(...)===true ашиглана (can() БИШ)');
  ok(/\$\{money \? `<button class="btn btn-primary" data-hourly-pay=/.test(src),
     'scan: «Цалин шилжүүлэх» товч мөнгөний эрхээр хаагдана');
  ok(/const paidLine = \(money && sum > 0\)/.test(src),
     'scan: авсан цалингийн мөр мөнгөний эрхээр хаагдана');
  ok(/const bankLine = !money \? ''/.test(src),
     'scan: банкны данс мөнгөний эрхээр хаагдана');
}
...
// 0j-3) Шөнийн агентын ажил нэг шөнөд ДУУСАХ хэмжээтэй байх (2026-09-16)
// Алдаа: суггестер хамгийн ИХ хуримтлагдсан функцийг (`kpi`, 200 inline style)
// нэрлэсэн. Агент 40 эргэлтийг дуусгаж унасан — бүр дуусгасан ч 500 мөрийн diff
// хязгаараас хэтэрч merge хийгдэхгүй байсан: суггестер ба merge-ийн хязгаар хоёр
// бие биенийхээ эсрэг ажиллаж байв. Дүрэм: санал болгосон ажил ҮРГЭЛЖ хүрээнд.
{
  const D = require('../tools/debt-check.js');
  const s = D.suggest();
  ok(D.SUG_MAX <= 30, 'шөнийн ажлын дээд хязгаар 30-аас хэтрэхгүй (diff хязгаарт таарна)');
  for (const r of s.top) {
    ok(r.count >= D.SUG_MIN && r.count <= D.SUG_MAX,
       `шөнийн ажил хүрээнд: ${r.fn} = ${r.count} (${D.SUG_MIN}–${D.SUG_MAX})`);
  }
  // Хүрээнээс том нь саналд ОРОХГҮЙ (тэднийг гараар хуваана)
  const names = new Set(s.top.map((r) => r.fn));
  eq(s.oversize.filter((r) => names.has(r.fn)).length, 0,
     'хэт том функц саналд орохгүй');
}
...
// 3b) Суурилуулалт токен + нэгж хөлс
{
  ...
  // Буцаан олголт токен + барьцааны буцаалт төрөл (C11)
  {
    ...
    // ⭐ НЭГ ГҮЙЛГЭЭ = БАРЬЦАА + ЗӨРҮҮ (2026-09-22, захиалга #1476)
    // Ажилтан барьцаа ба бараа багассаны зөрүүг НЭГ шилжүүлэгээр илгээдэг.
    // Токенд барьцааны ХЭСЭГ тусад нь үлдэхгүй бол зөрүү ч «барьцаа» болж өр уусна.
    {
      const n = F.encodeRefundNote('', 271000, 'сандал аваагүй зөрүү', 'dep', 150000);
      const r = F.parseRefund(n);
      eq({ amount: r.amount, kind: r.kind, dep: r.dep }, { amount: 271000, kind: 'dep', dep: 150000 },
         'Refund: нийт 271,000₮-ийн 150,000₮ нь барьцаа гэж тусад нь үлдэнэ');
      eq(F.parseRefund(F.encodeRefundNote('', 271000, '', 'dep', 400000)).dep, 271000,
         '⛔ барьцааны хэсэг нийтээс хэтрэхгүй');
      eq(F.parseRefund(F.encodeRefundNote('', 271000, '', '', 150000)).dep, 0,
         '⛔ арилжааны буцаалтад барьцааны хэсэг бичигдэхгүй');
      eq(F.parseRefund('⟦RF|150000||dep⟧').dep, 0, 'хуучин dep токенд 5-р талбар байхгүй');
    }
  }
}
...
// ТООГООР биш ӨРТГӨӨР хэмжинэ — 280 барааны 49 нь хөрөнгийн 80%.
{
  ...
  //    шалгах» — 1.2 тэрбум₮-ийн суурь дээр хангалтгүй.
  {
    ...
    ok(/захирал/.test(SB(only1, '88', true)), 'хориг: хэн батлахыг нэрлэнэ');
    ...
    // 🔧 ЭВДРЭЛИЙГ ЯЛГАХ (2026-09-22) — «106 ш» гэдэг нь 106 нь бүтэн гэсэн
    //   үг биш. Ялгахгүй бол суурь «бүгд бүтэн» гэж хөлдөж, боломжит нөөц
    //   хэтрэн харагдана (давхар захиалгын эх үүсвэр).
    const CS = vm.runInContext('openingCountSplit', sandbox);
    eq(CS(106, 6), { total: 106, damaged: 6, good: 100 }, 'эвдрэл: нийтээс дотор нь ялгана');
    eq(CS(10, 0), { total: 10, damaged: 0, good: 10 }, 'эвдрэл: 0 бол бүгд бүтэн');
    // ⛔ Эвдэрсэн нь нийтээс их бол НИЙТЭЭР хязгаарлагдана (сөрөг бүтэн гарахгүй).
    eq(CS(5, 9), { total: 5, damaged: 5, good: 0 }, 'эвдрэл: нийтээс хэтэрвэл тайрна');
    eq(CS(5, -3), { total: 5, damaged: 0, good: 5 }, 'эвдрэл: сөрөг → 0');
    eq(CS('8', '2'), { total: 8, damaged: 2, good: 6 }, 'эвдрэл: мөр оролт');
    eq(CS(null, null), { total: 0, damaged: 0, good: 0 }, 'эвдрэл: хоосон → унахгүй');
    eq(CS(7.6, 1.4), { total: 8, damaged: 1, good: 7 }, 'эвдрэл: бүхэл болгоно');
  }

  {
    const _s3 = fs.readFileSync(path.join(__dirname, '..', 'app.js'), 'utf8');
    // ⛔ Эвдэрсэн хэсэг нь ЗАСВАРЫН БҮРТГЭЛЭЭР тэмдэглэгдэнэ — нөөцөөс шууд
    //   хасвал «хаана байна» гэдэг нь алга болж, засаад сэргээх зам үгүй болно.
    ok(/ensureRepairRecord\(\{ sku: p\.sku[\s\S]{0,200}?orderNumber: 0/.test(_s3),
       'scan: эвдрэл засварын бүртгэлд очно');
    // ⛔ Тоолох товч шууд бүртгэхгүй — эвдрэлийг асуух цонх нээнэ.
    ok(/openOpeningCountModal\(sku, q\)/.test(_s3), 'scan: тоолох товч цонх нээнэ');
  }
...
// 40f) Борлуулалтын суваг тайлан — сайт vs ажилтан, 2026-09-аас
{
  ...
  ok(F.finIsNonExpense('6960'), 'зардал биш: 6960 дотоод шилжүүлэг/буцаалт');
...
// ── ЖИГНЭСЭН ДУНДАЖ ӨРТӨГ (2026-09-12) ──────────────────────────────────────
// Ижил барааг өөр үнээр дахин авахад ганц `cost` талбар хоёр үнийг барьж чадахгүй.
// Хуучныг үлдээвэл хөрөнгө дутуу, шинээр дарвал илүү — жигнэсэн дундаж л зөв.
{
  const r = F.blendCost(60, 950000, 6, 1150000);
  eq(r.qty, 66, 'дундаж: тоо нийлнэ');
  eq(r.cost, Math.round((60 * 950000 + 6 * 1150000) / 66), 'дундаж: ЖИГНЭСЭН');
  eq(r.cost, 968182, 'дундаж: бодит тоо');
  ok(r.cost !== Math.round((950000 + 1150000) / 2), 'дундаж: энгийн дундаж БИШ');
  ok(r.cost > 950000 && r.cost < 1150000, 'дундаж: хоёр үнийн хооронд');

  // Үнэ 0 = «мэдэгдэхгүй». 0-ээр дундажлавал өртөг хиймлээр буурна.
  eq(F.blendCost(0, 0, 10, 500000), { qty: 10, cost: 500000 }, 'дундаж: хуучин өртөггүй → шинэ үнэ');
  eq(F.blendCost(10, 500000, 5, 0), { qty: 15, cost: 500000 }, 'дундаж: шинэ үнэ 0 → хуучин хэвээр');
  eq(F.blendCost(10, 500000, 0, 900000), { qty: 10, cost: 500000 }, 'дундаж: тоо 0 → хуучин хэвээр');

  // Хог оролт дээр унахгүй
  eq(F.blendCost(0, 0, 0, 0), { qty: 0, cost: 0 }, 'дундаж: бүх тэг → 0 (тэгд хуваахгүй)');
  eq(F.blendCost(null, undefined, 'x', NaN), { qty: 0, cost: 0 }, 'дундаж: хог оролт → 0');
  eq(F.blendCost(-5, -100, 10, 200000), { qty: 10, cost: 200000 }, 'дундаж: сөрөг утга 0 болно');

  // Ижил үнээр авбал дундаж ӨӨРЧЛӨГДӨХГҮЙ
  eq(F.blendCost(20, 750000, 30, 750000).cost, 750000, 'дундаж: ижил үнэ → хэвээр');
}
...
{
  const o = (n, s, e, st) => ({ id: 'o' + n, number: n, customer: 'Х', status: st || 'reserved', starts_at: s, stops_at: e });
  const d = F.ordersCalendarData([
    o(1, '2026-09-23', '2026-09-25'),
    o(2, '2026-09-23', '2026-09-23'),
    o(3, '2026-08-30', '2026-09-02'),      // өмнөх сард гарч, энэ сард буцна
    o(4, '2026-09-10', '2026-10-05'),      // энэ сард гарч, дараа сард буцна
    o(5, '2026-09-23', '2026-09-24', 'deleted'),
  ], '2026-09');
  eq((d.out['2026-09-23'] || []).length, 2, 'календарь: гарах өдрөөр');
  eq((d.back['2026-09-25'] || []).length, 1, 'календарь: буцах өдрөөр');
  eq((d.back['2026-09-02'] || []).length, 1, 'календарь: өмнө гарсан нь буцах өдрөөрөө орно');
  eq(d.out['2026-08-30'], undefined, 'календарь: өөр сарын гарах өдөр орохгүй');
  eq(d.back['2026-10-05'], undefined, 'календарь: өөр сарын буцах өдөр орохгүй');
  // ⛔ Олон өдрийн захиалгыг ДУНД өдрүүдэд давтахгүй — эс бөгөөс 10 хоногийн
  //    түрээс календарийг дүүргэж бодит ачаалал живнэ.
  eq(d.out['2026-09-24'], undefined, 'календарь: дунд өдөрт давтахгүй');
  eq((d.out['2026-09-23'] || []).some(x => x.number === 5), false, 'календарь: больсон захиалга орохгүй');

  const c = F.calendarCells('2026-09');
  eq(c.length, 35, 'календарь: 9 сар 5 мөр');
  eq(c[0], null, 'календарь: Даваагаар эхэлнэ (9-р сар Мягмараас)');
  eq(c[1], '2026-09-01', 'календарь: эхний өдөр');
  eq(c[c.length - 5], '2026-09-30', 'календарь: сүүлийн өдөр');
  eq(F.calendarCells('').length, 0, 'календарь: буруу сар → хоосон');
}
...
// ── ГАРАХ ЁСТОЙ ЦАГ — хоцролтын хэмжүүр (2026-09-22) ───────────────────────
{
  ...
  const cityPlan = F.orderDispatchPlan(mk(1, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧'));
  eq(cityPlan.hours, 2.5, 'гарах: хот = 2.5 цаг (замын доод хэмжээтэй)');
  // 220 км = 3.67 цаг зам + 1 + 0.5
  eq(F.orderDispatchPlan(mk(2, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|out|220|0⟧')).hours, 5.2,
     'гарах: 220 км = 5.2 цаг');
  // Суурилуулалт нэмнэ
  eq(F.orderDispatchPlan(mk(3, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|city|40|0⟧ ⟦SET|1⟧')).hours, 3.5,
     'гарах: 40 км + угсралт = 3.5 цаг (хотод замын доод хэмжээ ялна)');
  // ⛔ Өөрсдөө авах (pickup) бол ЗАМ ТООЦОХГҮЙ — км нь хүргэлтийнх биш.
  eq(F.orderDispatchPlan(mk(4, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|pickup|220|0⟧')).km, 0,
     'гарах: pickup-д зам тооцохгүй');
  // ⛔ Эхлэх цаггүй бол ТААМАГЛАХГҮЙ.
  ...
  // pickup — зам тооцохгүй тул 1.5 цаг л хангалттай
  eq(F.orderDispatchPlan(mk(10, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|pickup|0|0⟧')).hours, 1.5,
     'гарах: өөрсдөө авах = 1.5 цаг');
  ...
  const st = F.dispatchStats([
    mk(6, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧', '2026-09-14T07:29:00Z'),
    mk(7, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧', '2026-09-14T02:00:00Z'),
    mk(8, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧', '2026-09-17T09:00:00Z'),
    mk(9, '2026-09-14', '', '2026-09-14T02:00:00Z'),
  ], '2026-09-01');
  eq(st.n, 2, 'нэгтгэл: хэмжигдсэн тоо');
  eq(st.late, 1, 'нэгтгэл: хоцорсон тоо');
  eq(st.pct, 50, 'нэгтгэл: цагтаа хувь');
  eq(st.wild, 1, 'нэгтгэл: бүртгэлийн алдаа тусад нь');
  eq(st.skipped, 1, 'нэгтгэл: цаггүй захиалга тусад нь');
  ...
  const dayRows = F.dispatchDayRows([
    ...
    mk(12, '2026-09-14', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧', '2026-09-14T02:00:00Z'),   // цагтаа
    mk(13, '2026-09-16', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧', '2026-09-16T02:00:00Z'),
    mk(14, '2026-08-30', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧', '2026-08-30T02:00:00Z'),   // өөр сар
    mk(15, '2026-09-18', '⟦RT|13|18⟧ ⟦DLV|city|0|0⟧', '2026-09-25T09:00:00Z'),   // хожуу дарсан
  ], '2026-09');
  eq(dayRows.length, 2, 'өдөр: зөвхөн хэмжигдсэн өдөр');
  eq(dayRows[0].day, '2026-09-14', 'өдөр: огноогоор эрэмбэлэгдсэн');
  eq(dayRows[0].n, 2, 'өдөр: тухайн өдрийн тоо');
  eq(dayRows[0].late, 1, 'өдөр: хоцорсон тоо');
  eq(dayRows[0].orders[0].number, 11, 'өдөр: хоцорсон захиалга нэрлэгдэнэ');
  eq(dayRows[1].late, 0, 'өдөр: цагтаа өдөрт хоцролт 0');
  eq(F.dispatchDayRows([], '2026-09').length, 0, 'өдөр: хоосон → унахгүй');

  const _s2 = fs.readFileSync(path.join(__dirname, '..', 'app.js'), 'utf8');
...
// «ЗАРДАЛ БИШ» БҮЛГИЙГ ЯЛГАХ (2026-09-11)
// Эзний зээл(69xx) ба барьцаа буцаалт(5810) нь мөнгө гарсан ч ашгийг бууруулдаггүй.
// Гүйлгээний жагсаалтад жинхэнэ зардалтай ижил харагдах нь төөрөгдүүлдэг байв.
// ⚠ Холимог бүлгийг саармаг гэж буудвал жинхэнэ зардал нуугдана — БҮХ мөр саармаг байх ёстой.
{
  const NE = vm.runInContext('finIsNonExpense', sandbox);
  const DR = vm.runInContext('finIsDepositReturn', sandbox);
  const neutral = (arr) => !!(arr && arr.length) && arr.every(t => NE(t.category) || DR(t));

  ok(neutral([{ category: '6900' }, { category: '6950' }]), 'саармаг: 69xx бүлэг');
  ok(neutral([{ category: '5810' }]), 'саармаг: барьцаа буцаалт');
  ok(neutral([{ category: '6960' }, { category: '5810' }]), 'саармаг: холимог саармаг ч саармаг');
  ok(!neutral([{ category: '6900' }, { category: '1300' }]), 'саармаг: нэг ч жинхэнэ зардал байвал саармаг БИШ');
  ok(!neutral([{ category: '6100' }]), 'саармаг: хөрөнгө(6100) нь ЗАРДАЛ — саармаг биш');
  ok(!neutral([{ category: '5800' }]), 'саармаг: 5800 нь мөрөөс хамаарна — бүлгээр буудахгүй');
  ok(!neutral([]), 'саармаг: хоосон бүлэг → үгүй');
  ok(!neutral(null), 'саармаг: null → унахгүй');
}

// SCAN — саармаг бүлгийн загвар CSS-д байна (2026-09-11)
{
  const css = fs.readFileSync(path.join(__dirname, '..', 'styles.css'), 'utf8');
  ok(/\.fin-grp-neutral/.test(css), 'scan: fin-grp-neutral загвар тодорхойлогдсон');
  ok(/fin-grp-neutral[\s\S]{0,400}зардалд ОРОХГҮЙ/.test(css), 'scan: саармаг бүлэгт тайлбар шошго гарна');
}

// ЦАЛИНГИЙН АНГИЛАЛ — «код» ба «код + нэр» хоёулаа таарна (2026-09-11)
// Ангилал DB-д хоёр хэлбэрээр хадгалагдсан: «7200» (1167 мөр) ба
// «7200 Цагийн / улирлын ажилтны цалин» (75 мөр). Шалгалт мөрийн төгсгөлд
// уягдсан ($) байсан тул урт хэлбэртэй 24 цалингийн гүйлгээ (6.7 сая₮)
// тайлангийн цалингийн нийлбэрээс чимээгүй унаж байв.
{
  const IS = vm.runInContext('finIsSalaryCat', sandbox);
  eq(IS('7100'), true, 'цалингийн ангилал: зөвхөн код');
  eq(IS('7200 Цагийн / улирлын ажилтны цалин'), true, 'цалингийн ангилал: код + нэр');
  eq(IS('7100 Үндсэн ажилтны цалин'), true, 'цалингийн ангилал: 7100 урт хэлбэр');
  eq(IS('7300'), true, 'цалингийн ангилал: 7300 гэрээт');
  eq(IS('7600'), true, 'цалингийн ангилал: 7600 бусад');
  eq(IS('7400'), false, 'цалингийн ангилал: 7400 бонус нь цалингийн нийлбэрт ОРОХГҮЙ');
  eq(IS('7700'), false, 'цалингийн ангилал: 7700 ашгийн урамшуулал ОРОХГҮЙ');
  eq(IS('1800'), false, 'цалингийн ангилал: шатахуун БИШ');
  eq(IS(''), false, 'цалингийн ангилал: хоосон → үгүй');
  eq(IS(null), false, 'цалингийн ангилал: null → унахгүй');
}

// SCAN — ангиллын шалгалт мөрийн төгсгөлд уягдахгүй (2026-09-11)
{
  eq((src.match(/\^7\[1236\]00\$/g) || []).length, 0,
    'scan: цалингийн ангилал $-аар уягдаагүй — finIsSalaryCat() ашиглана');
}
...
{
  // 2026-09-12: эможи → SVG дүрс (rowIcon). Шошго өөрөө хэвээр.
  ok(/rowIcon\('mail'\)\}Илгээгээгүй/.test(src), 'scan: илгээгээгүй ноорогт тэмдэг гарна');
...
// scan: нийтлэгч — мөнгө хөндөх, гадагш чиглэсэн скрипт тул дүрмүүд нь
// кодод бэхлэгдсэн байх ёстой.
{
  const py = fs.readFileSync(path.join(__dirname, '..', 'tools', 'fb_publish.py'), 'utf8');

  // ⛔ Зөвхөн БАТАЛСАН мөр. Ноорог/болисон пост хэзээ ч нийтлэгдэхгүй.
  ok(/status = 'approved'/.test(py), 'scan: нийтлэгч зөвхөн approved мөрийг авна');
  ok(!/status = 'draft'/.test(py), 'scan: ноорог постыг хөндөхгүй');

  // ⛔ Зар унтраалттай үед юу ч хийхгүй — «Бүх зар зогсоо» дарсан хүн пост
  //   нийтлэгдэж мөнгө гарна гэж бодохгүй.
  ok(/if not on:[\s\S]{0,200}?sys\.exit\(0\)/.test(py),
     'scan: зар унтраалттай бол нийтлэхгүй гарна');
  ok(/budget\.get\('enabled'\)/.test(py) && /budget\.get\('month'\)/.test(py),
     'scan: унтраалт БА сарыг хоёуланг шалгана');

  // ⛔ Зорилтот бүлгийг ЗОХИОХГҮЙ — ажиллаж байгаа зараас хуулна.
  ok(/tpl\.get\('targeting'\)/.test(py), 'scan: таргетийг загвараас хуулна');
  ok(/if not sets:[\s\S]{0,200}?sys\.exit\(0\)/.test(py),
     'scan: загвар олдохгүй бол таамаглахгүй, зогсоно');

  // ⚠ Хэсэгчилсэн бүтэлгүйтэл: пост нийтлэгдээд зар үүсэхгүй байж болно.
  //   `fb_post_id` нь зар үүсгэхийн ӨМНӨ хадгалагдана — эс бөгөөс дараагийн
  //   оролдлого ижил постыг ДАХИН нийтэлнэ.
  const iSave = py.indexOf("set fb_post_id=");
  const iCamp = py.indexOf("/campaigns'");
  ok(iSave > 0 && iCamp > 0 && iSave < iCamp,
     'scan: fb_post_id зар үүсгэхийн ӨМНӨ хадгалагдана (пост давхардахгүй)');
  ok(/if not post_id:/.test(py), 'scan: нийтлэгдсэн постыг дахин нийтлэхгүй');
...
// зарын нэрэнд хашилт байдаг тул шууд наавал SQL эвдэрнэ.
{
  ...
  // ⛔ Алдаа нь ӨӨРЧЛӨЛТ биш ТӨЛӨВ — засагдтал 10 минут тутам давтагдана.
  // Хаалтгүй бол өдөрт 144 ижил мөр бичигдэж бүртгэл ашиггүй болно
  // (амьд системд 2026-09-16-нд яг ингэж эхэлсэн).
  ok(/if k == 'error':[\s\S]{0,300}?not exists \(select 1 from fb_ad_actions/.test(py),
     'scan: давтагдсан алдааг дахин бичихгүй (хаалт нь алдааны замд байна)');
...
// ═══════════════════════════════════════════════════════════════════════════
{
  ...
  const over   = sandbox.orderOverpaid;
  ...
  // ━━━ НЭГ БУЦААЛТАД ХОЁР ЗҮЙЛ (2026-09-22, захиалга #1476) ━━━
  // Барьцаа 150,000₮ + сандал аваагүйн зөрүү 121,000₮ = 271,000₮ НЭГ шилжүүлэгээр.
  // Токенд барьцааны хэсэг тусад нь байхгүй бол бүх 271,000₮ «барьцаа» гэж уншигдаж,
  // барьцаагаар таглагдахад зөрүү алга болж өр буцаад үүснэ.
  {
    // (а) ЗАХИАЛГА ЗАСАГДСАН — сандал хасагдаж total 500,000 → 379,000
    const fixed = O({ total_mnt: 379000, paid_mnt: 229000, deposit_mnt: 150000,
                      note: '⟦RF|271000|сандал аваагүй|dep|150000⟧' });
    eq(refDep(fixed), 150000, '#1476: зөвхөн барьцааны 150,000₮ хасагдана');
    eq(billed(fixed), 229000, '#1476: авах ёстой нийт = 229,000₮');
    eq(owed(fixed), 0,        '#1476: ҮЛДЭГДЭЛ 0 — «дутуу» гэж харагдахаа болино');
    eq(over(fixed), 0,        '#1476: илүү төлөлт ч алга');

    // (б) ЗАХИАЛГА ЗАСАГДААГҮЙ — зөрүүг буцаасан боловч total хүчингүй
    const stale = O({ total_mnt: 500000, paid_mnt: 229000, deposit_mnt: 150000,
                      note: '⟦RF|271000|сандал аваагүй|dep|150000⟧' });
    eq(owed(stale), 121000,
       '⛔ захиалга засагдаагүй бол 121,000₮ дутуу гэж ХЭЛНЭ — чимээгүй залгихгүй');

    // (в) БҮХ дүнг барьцаа гэж тэмдэглэвэл (хуучин зам) — зөрүү алга болно
    const wrong = O({ total_mnt: 379000, paid_mnt: 229000, deposit_mnt: 150000,
                      note: '⟦RF|271000||dep⟧' });
    eq(owed(wrong), 0, 'хуучин токен: барьцаагаар таглагдаад асуудалгүй (нийцтэй)');

    // (г) БАРЬЦАА ХЭСЭГЧИЛЭН буцаасан (эвдрэл суутгасан) + зөрүү нэг гүйлгээгээр.
    //    Барьцаа 200,000₮-ийн 150,000₮ л буцсан; үлдсэн 121,000₮ нь зөрүү.
    //    Задаргаагүй бол бүх 271,000₮ барьцаа гэж уншигдаж 200,000₮ хасагдана (50,000₮ алдаа).
    const split = O({ total_mnt: 500000, paid_mnt: 229000, deposit_mnt: 200000,
                      note: '⟦RF|271000|зарим барьцаа суутгав|dep|150000⟧' });
    eq(refDep(split), 150000,
       '⛔ хэсэгчилсэн барьцаа: 150,000₮ л хасагдана (бүх 271,000₮ БИШ)');
    eq(owed(split), 121000, '⛔ зөрүүг барьцаа гэж үзээд өр чимээгүй уучлагдахгүй');
  }

  // ━━━ ИЛҮҮ ТӨЛӨЛТ — багассан захиалгын чимээгүй үлдсэн мөнгө ━━━
  ok(typeof over === 'function', 'илүү төлөлт: orderOverpaid бий');
  eq(over(O({ total_mnt: 379000, paid_mnt: 500000, deposit_mnt: 150000 })), 121000,
     'бараа хасагдсан: 121,000₮ илүү төлсөн гэж гарна');
  eq(over(O({ total_mnt: 500000, paid_mnt: 500000, deposit_mnt: 150000 })), 0,
     'хэвийн бүрэн төлөлт: илүү төлөлт алга');
  eq(over(O({ total_mnt: 500000, paid_mnt: 300000 })), 0, 'дутуу төлсөн: илүү төлөлт 0');
  eq(over(null), 0, 'илүү төлөлт: хог оролтод унахгүй');
  // Барьцаа буцаасны ДАРАА илүү төлөлт ХИЙМЭЛ үүсэхгүй
  eq(over(O({ total_mnt: 379000, paid_mnt: 229000, deposit_mnt: 150000,
              note: '⟦RF|271000||dep|150000⟧' })), 0,
...
// 196,280₮ хөнгөлсөн гэж бодсон ч `orderDiscountAmount` = max(авто, гар) тул
// ЧИМЭЭГҮЙ хаягдсан. Дүн 12,196,280 хэвээр, харилцагч 12,000,000 төлсөн →
// 196,280₮ «дутуу». Одоо эцсийн дүнгээ бичвэл систем хөнгөлөлтийг УРВУУ бодно.
// ═══════════════════════════════════════════════════════════════════════════
{
  const dff = sandbox.discountForFinal;
  const oda = sandbox.orderDiscountAmount;
  ok(typeof dff === 'function', 'тохирсон дүн: discountForFinal бий');

  // БОДИТ КЕЙС 1532 — НӨАТ хөнгөлөлттэй, хүргэлт 2,200,000₮
  {
    const sub = 13153000, fixed = 2200000, F = 12000000;
    const d = dff(sub, F, true, fixed);
    const rentalNet = sub - d;
    const vat = Math.round(rentalNet * 0.05);
    eq(Math.max(0, rentalNet - vat) + fixed, F, '1532: эцсийн дүн ЯГ 12,000,000₮ гарна');
    ok(d > Math.round(sub * 0.20), '1532: хөнгөлөлт авто 20%-аас ИЛҮҮ — хэрэглэгдэнэ');
    // max(авто, гар) шүүлтээр өнгөрсний дараа хүртэл ижил байна
    eq(oda(sub, 2, 'amount', d), d, '1532: бодогдсон хөнгөлөлт max() шүүлтээр өөрчлөгдөхгүй');
  }

  // НӨАТГҮЙ — харьцангуй хялбар
  {
    const d = dff(1000000, 900000, false, 100000);
    eq(d, 200000, 'НӨАТгүй: 1,000,000 + 100,000 тогтмол → 900,000 болгоход 200,000₮ хөнгөлнө');
    eq(1000000 - d + 100000, 900000, 'НӨАТгүй: эцсийн дүн таарна');
  }

  // ⛔ БӨӨРӨНХИЙЛӨЛТ — НӨАТ-тай үед ЯГ таарах ёстой (1₮ ч зөрөхгүй)
  for (const F of [12000000, 9999999, 5000001, 777777, 100003]) {
    const sub = 13153000, fixed = 2200000;
    if (F <= fixed) continue;
    const d = dff(sub, F, true, fixed);
    const rn = sub - d, v = Math.round(rn * 0.05);
    eq(Math.max(0, rn - v) + fixed, F, `бөөрөнхийлөлт: ${F.toLocaleString()}₮ ЯГ таарна`);
  }

  // Эцсийн дүн тогтмол зардлаас бага → бүх түрээсийг хөнгөлнө (сөрөг болохгүй)
  eq(dff(1000000, 50000, false, 100000), 1000000, 'эцсийн дүн хэт бага: бүх түрээс хөнгөлөгднө');
  eq(dff(1000000, 5000000, false, 0), 0, 'эцсийн дүн их: хөнгөлөлт 0 (сөрөг болохгүй)');
  eq(dff(0, 0, false, 0), 0, 'хог оролт: 0');

  // Хөнгөлөлтийн шалтгаан
  const dr = sandbox.orderDiscountReason;
  eq(dr({ stage_meta: { discount: { reason: 'тогтмол харилцагч' } } }), 'тогтмол харилцагч', 'хөнгөлөлтийн шалтгаан уншигдана');
  eq(dr(null), '', 'хөнгөлөлтийн шалтгаан: хог оролтод хоосон');
}

// ━━━ SCAN: ХӨНГӨЛӨЛТ ЧИМЭЭГҮЙ ХАЯГДАХГҮЙ (2026-09-22) ━━━
{
  const at = src.indexOf('function openNewOrder');
  const fn = src.slice(at, at + 56000);
  ok(/id="no-disc-hint"/.test(fn), 'scan: хөнгөлөлт хэрэглэгдсэн эсэхийг ил хэлэх мөр бий');
  ok(/value="final"/.test(fn), 'scan: «Тохирсон дүн» сонголт бий');
  // ⛔ ШАЛТГААН = ЦОНХООР. Модал доторх нуугдах талбар байсан — хэрэглэгч
  //    ОЛДОХГҮЙ байсан ба нуугдсан үед шаардлага ажиллавал ГАЦААДАГ байв (2026-09-22).
  //    Цонх нь CSS/байрлал/нуултаас ОГТ хамаарахгүй — үргэлж гарч ирнэ.
  ok(!/no-disc-why/.test(src),
     '⛔ scan: нуугдах шалтгааны ТАЛБАР буцаж ирээгүй (цонхоор асууна)');
  ok(/if \(_manualDiscUsed\) \{\s*\n\s*const _ans = await showPrompt\(/.test(fn),
     'scan: гар хөнгөлөлтийн шалтгаан ЦОНХООР асуугдана');
  ok(/defaultValue: _discWhy/.test(fn),
     'scan: засахад өмнөх шалтгаан цонхонд бөглөгдсөн ирнэ');
  // ⛔ ШАЛТГААН АЖЛЫГ ЗОГСООХГҮЙ (2026-09-22, CEO гомдол).
  //    Анх хоосон бол `return` хийдэг байсан — хөнгөлөлт өгөх гэсэн хүн
  //    захиалгаа хадгалж ЧАДАХГҮЙ болж амьд системд ажил зогссон.
  ok(/if \(_ans === null\) return;/.test(fn),
     '⛔ scan: шалтгаан заавал БИШ — зөвхөн Цуцлах дарвал зогсоно');
  ok(!/шалтгаан бичээгүй — хадгалаагүй/i.test(fn),
     '⛔ scan: хоосон шалтгаанаас болж хадгалахгүй болох зам буцаж ирээгүй');
  // ⛔ DB-д 'final' төрөл БАЙХГҮЙ — карт/тайлан танихгүй төрлөөр буруу бодно
  ok(/discount_type: \(dtype === 'final' \? \(discount > 0 \? 'amount' : null\)/.test(fn),
     '⛔ scan: «final» нь DB-д amount болж хадгалагдана');
  // Хадгалахад recalc-ийн ДҮН ашиглана — дахин бодвол зөрнө
  ok(/const discount = _discNow;/.test(fn),
     'scan: хадгалахад дэлгэц дээр бодогдсон хөнгөлөлт ашиглагдана');
}
...
// Баримт мартагддаг; энэ тест мартагддаггүй.
{
  ...
  ok(/rf\.dep/.test(helpers),
     'scan: нэг буцаалтын БАРЬЦААНЫ хэсэг тусад нь уншигдана (зөрүү барьцаа болж өр идэхгүй)');
}

// ━━━ SCAN: БУЦААЛТЫН ЗАДАРГАА БУЦАЖ ХААГДАХГҮЙ (2026-09-22) ━━━
// Чагт («энэ нь барьцааны буцаалт») нь ХАГАС асуулт байв: нэг шилжүүлэгт
// барьцаа БА зөрүү хоёуланг нь ордог. Дараагүй бол өр дутуу гэж үлдэнэ (#1476).
{
  const at = src.indexOf('function openRefundModal');
  ok(at > 0, 'scan: openRefundModal олдов');
  const fn = src.slice(at, at + 16000);
  ok(/id="rf-dep"/.test(fn), 'scan: буцаалтын модалд «үүнээс барьцаа» дүнгийн талбар бий');
  ok(!/rf-isdep/.test(src),
     'scan: хуучин чагт буцаж ирээгүй — задаргаа байх ёстой');
  ok(/encodeRefundNote\(baseNote, prevRfTot \+ refundAmt, userNote, isDep \? 'dep' : '', depPart\)/.test(fn),
     'scan: барьцааны хэсэг токенд бичигдэнэ');
  ok(/prevDepPart/.test(fn),
     'scan: өмнөх буцаалтын барьцаа дээр нэмэгдэнэ (хэсэгчилсэн буцаалт алдагдахгүй)');
}

// ━━━ SCAN: ИЛҮҮ ТӨЛӨЛТ НУУГДАХГҮЙ (2026-09-22) ━━━
// `orderOwed` нь 0-гоор тагладаг тул захиалга багасахад үүссэн илүү төлөлт
// ХААНА Ч харагдахгүй байв — үйлчлүүлэгчийн мөнгө чимээгүй үлдэх нүх.
{
  ok(/function orderOverpaid\(o\)/.test(src), 'scan: orderOverpaid ганц эх сурвалж бий');
  const at = src.indexOf('function bqOrderCard');
  const card = src.slice(at, at + 12000);
  ok(/orderOverpaid\(o\)/.test(card), 'scan: захиалгын карт илүү төлөлтийг шалгана');
  ok(/op-over/.test(card), 'scan: илүү төлөлт төлбөрийн самбарт ил гарна');
  // Захиалгыг засаад дүн буурвал буцаалтын модал ӨӨРӨӨ санал болгоно — хүн мартахгүй
  const sv = src.indexOf('await saveAppOrder(ord)');
  ok(sv > 0 && /orderOverpaid\(ord\)/.test(src.slice(sv, sv + 1400)),
     'scan: захиалга засагдсны дараа илүү төлөлт шалгагдана');
  ok(/openRefundModal\(ord\.id\)/.test(src.slice(sv, sv + 1400)),
     'scan: илүү төлөлт үүсвэл буцаалтын модал шууд нээгдэнэ');
}
...
// холбогдоогүй орсон мөнгө хаана ч үлддэггүй. Эдгээр тест тэр 2-ыг хамгаална.
{
  ...
  // ── ЗАЛГАА: дутуу хугацаа = мөнгө чимээгүй алга болох цорын ганц бодит эрсдэл ──
  {
    ...
    /* ⭐ ХУВИЙН ДАНС сар хаахыг блоклохгүй (2026-09-11). Эзний хувийн данс 455сая
       эргэлттэй, 534,683₮ зөрүүтэй байхад компанийн сар хаагдахгүй байв — гэтэл
       тэр зөрүү компанийн тайланд огт нөлөөлөхгүй. */
    const P = { ...A, id: 'p', acct: '9911223344', closing_calc: 900 };   // хувийн данс, тэнцэл зөрүүтэй
    eq(F.stmtChainCheck([E, P]).map(x => x.acct), ['504', '9911223344'],
       'залгаа: шүүлтгүй үед бүх данс шалгагдана (тулгалтын цонх бүгдийг харуулна)');
    eq(F.stmtChainCheck([E, P], ['504']).map(x => x.acct), ['504'],
       'залгаа: сонгосон данс — хувийн дансны зөрүү гарахгүй');
    // Банк болгон дугаарыг өөрөөр бичдэг — сүүлийн 10 оронгоор тулгана
    const L = { ...A, id: 'L', acct: '5400145457', closing_calc: 900 };
    eq(F.stmtChainCheck([L], ['5400-145-457']).map(x => x.acct), ['5400145457'],
       'залгаа: зураастай бичсэн данс ч тулгагдана');
    eq(F.stmtChainCheck([L], ['0015400145457']).map(x => x.acct), ['5400145457'],
       'залгаа: урдаа тэгтэй бичсэн данс ч тулгагдана (сүүлийн 10 орон)');
    eq(F.stmtChainCheck([E, P], []), [], 'залгаа: хоосон жагсаалт → юу ч шалгахгүй');
  }

  // ── Тэнцлийн зөрүү ЮУ дутсаныг хэлнэ ──
  {
    need(['gapBalanceWhy']);
    ok(/ЗАРЛАГА/.test(F.gapBalanceWhy(1070023)), 'эерэг зөрүү = зарлага уншигдаагүй');
    ok(/ОРЛОГО/.test(F.gapBalanceWhy(-534683)), 'сөрөг зөрүү = орлого уншигдаагүй');
    ok(/534,683/.test(F.gapBalanceWhy(-534683)), 'дүн нь эерэгээр бичигдэнэ (хасах тэмдэггүй)');
    ok(!/ЗАРЛАГА|ОРЛОГО/.test(F.gapBalanceWhy(0)), 'тэг зөрүү → чиглэл заахгүй');
  }
...
// ═══ БАНКНЫ ШИМТГЭЛ = ЗАРДАЛ (2026-09-11) ═════════════════════════════════════
// Өмнө нь хуулга оруулахад бүрмөсөн хасагддаг байсан тул тайлангаас чимээгүй
// унадаг байв — 11 хоногт 26 мөр · 4,800₮ (амьд Голомт хуулгаас тоолов).
function testBankFee() {
  need(['isBankFee', 'classifyExpense']);
  for (const m of ['Charges for PORD Customer Payment :000641721544',
                   'Гүйлгээний  шимтгэл', 'ГҮЙЛГЭЭНИЙ ШИМТГЭЛ', 'charges for pord']) {
    ok(F.isBankFee(m), 'шимтгэл танигдана: ' + m.slice(0, 34));
    eq(F.classifyExpense(m, ''), '5700', 'шимтгэл → 5700 ангилал: ' + m.slice(0, 34));
  }
  for (const m of ['ЗАРЛАГА БҮҮСТ ТӨЛБӨР', 'EB-Зарлага: Портер түлш', '', 'ДАНС ХООРОНД']) {
    ok(!F.isBankFee(m), 'энгийн зардал шимтгэл БИШ: ' + (m || '(хоосон)'));
  }
  // ⚠ Нийгмийн даатгалын шимтгэл ч энэ дүрэмд орно — ангилал нь 5700 болох нь
  //   буруу ч гэсэн зардлаас УНАХААС дээр. Эзэн Миний зардлаас засна.
  ok(F.isBankFee('НДШ шимтгэл'), 'шимтгэл гэсэн үг агуулсан бүхэн танигдана (алдагдахаас дээр)');

  /* ⛔ ШИМТГЭЛ дотоод шилжүүлгээс ӨМНӨ шалгагдана. «Charges for PORD» мөрийн
     харьцсан данс нь ХҮЛЭЭН АВАГЧИЙНХ — өөрийн данс руу шилжүүлэхэд тэр нь
     бидний данс болж, шимтгэл «дотоод шилжүүлэг» гэж хасагдаж байв (амьд
     дэлгэцээс олов: «Charges for … 200₮ · дотоод шилжүүлэг → Хаан ••3015»). */
  {
    const at = src.indexOf('const parsed = st.rows.filter');
    ok(at > 0, 'scan: зарлагын шүүлтүүр олдов');
    const body = src.slice(at, at + 1200);
    const feeAt = body.indexOf('isBankFee(r.memo)');
    const intAt = body.indexOf('isInternalTransfer(r, own)');
    ok(feeAt > 0 && intAt > 0 && feeAt < intAt,
...
function testCatResolverSkip() {
  need(['_histCatResolver']);
  const prods = [{ sku: 'M-101', name: 'PAR гэрэл', category: 'Тайзны гэрэлтүүлэг' }];
  /* ⚠ Нэрийн түлхүүрийг ГАРААР бүү бич — `normItemKey` нь ө/ү зэрэг үсгийг хасдаг
     («НӨАТ» → «нат»). Функцээр гаргаж байж л бодит зантай таарна. */
  const aliases = {
    'sku:b64d2879': '',                            // хүргэлт — бараа биш
    ['name:' + F.normItemKey('НӨАТ')]: '',         // НӨАТ — бараа биш
    'sku:old-par': 'M-101',                        // хуучин sku → каталогийн бараа
  };
  const catOf = F._histCatResolver(prods, aliases);

  eq(catOf('M-101', 'PAR гэрэл'), 'Тайзны гэрэлтүүлэг', 'шууд таарсан');
  eq(catOf.stats.exact, 1, 'шууд таарсан нь exact');

  eq(catOf('old-par', 'ямар нэг хуучин нэр'), 'Тайзны гэрэлтүүлэг', 'толиор тулгагдана');
  eq(catOf.stats.alias, 1, 'толиор тулгагдсан нь alias');

  // ⭐ Бараа биш — skip болно, guess/none-д ОРОХГҮЙ (анхааруулга гарахгүй)
  catOf('B64D2879', '2 талдаа Хүргэлт  (машины төрөл дунд)');
  eq(catOf.stats.skip, 1, 'бараа биш (sku толь) → skip');
  eq([catOf.stats.guess, catOf.stats.none], [0, 0], 'бараа биш нь анхааруулгад тоологдохгүй');

  catOf('', 'НӨАТ');
  eq(catOf.stats.skip, 2, 'бараа биш (нэрийн толь) → skip');
  eq([catOf.stats.guess, catOf.stats.none], [0, 0], 'нэрээр тэмдэглэсэн нь ч анхааруулгад орохгүй');

  // Тэмдэглээгүй танихгүй бараа нь ХЭВЭЭР анхааруулгад орно
  catOf('ZZZ', 'Огт танихгүй зүйл 12345');
  eq(catOf.stats.none, 1, 'тэмдэглээгүй танихгүй мөр анхааруулгад үлдэнэ');
}
testCatResolverSkip();

// ═══ ОРЛОГЫН ЗӨРҮҮГ ТАЙЛБАРЛАХ (2026-09-12) ══════════════════════════════════
// «Нийт орлого 849сая» ба «Нөхсөн 648сая» зэрэг харагдаж, хэрэглэгч алдаа гэж
// бодож байв. Хоёр тоо өөр зүйл хэмждэг — зөрүүг задалж ил бичнэ.
function testRevGap() {
  need(['revGapHtml']);
  const bq = {
    summary: { net_revenue_mnt: 849005157 },
    services: [{ revenue_mnt: 120000000 }, { revenue_mnt: 30000000 }],
    unknownRev: 20000000,
  };
  const h = F.revGapHtml(bq, 648477323);
  ok(/849,005,157/.test(h), 'зөрүү: нийт орлого бичигдэнэ');
  ok(/648,477,323/.test(h), 'зөрүү: барааны түрээс бичигдэнэ');
  ok(/200,527,834/.test(h), 'зөрүү: ялгаа бодогдоно');
  ok(/үйлчилгээ/.test(h) && /150,000,000/.test(h), 'зөрүү: үйлчилгээний дүн нийлнэ');
  ok(/тулгагдаагүй/.test(h) && /20,000,000/.test(h), 'зөрүү: тулгагдаагүй орлого тусад нь');
  ok(/30,527,834/.test(h), 'зөрүү: үлдсэн нь «хүргэлт/бусад» болж ҮЛДЭХГҮЙ алга болохгүй');

  // Хугацааны шүүлт идэвхтэй үед тайлбар гарахгүй (харьцуулалт утгагүй)
  eq(F.revGapHtml({ ...bq, _full: false }, 1), '', 'зөрүү: хугацааны шүүлттэй үед тайлбар гарахгүй');
  // Зөрүүгүй / сөрөг бол ч гарахгүй
  eq(F.revGapHtml({ summary: { net_revenue_mnt: 100 } }, 100), '', 'зөрүү: тэнцүү бол тайлбаргүй');
  eq(F.revGapHtml({ summary: { net_revenue_mnt: 100 } }, 200), '', 'зөрүү: сөрөг бол тайлбаргүй');
  eq(F.revGapHtml(null, 5), '', 'зөрүү: хоосон → хоосон');
  // Үйлчилгээ/тулгагдаагүй мэдэгдэхгүй ч зөрүү нь ил гарна
  const bare = F.revGapHtml({ summary: { net_revenue_mnt: 500 } }, 300);
  ok(/хүргэлт\/бусад мөр/.test(bare) && /200/.test(bare),
     'зөрүү: задаргаагүй үед бүх зөрүү «хүргэлт/бусад» болж ил гарна');
}
testRevGap();
...
function testDropGroups() {
  need(['dropReasonGroups']);
  const rows = [
    { why: 'банкны шимтгэл', debit: 200 },
    { why: 'дотоод шилжүүлэг (өөрийн данс)', debit: 1500000 },
    { why: 'банкны шимтгэл', debit: 200 },
    { why: 'дотоод шилжүүлэг (өөрийн данс)', debit: 500000 },
    { why: 'огноо уншигдсангүй', debit: 3600 },
  ];
  const g = F.dropReasonGroups(rows);
  eq(g.map(x => x.why), ['дотоод шилжүүлэг (өөрийн данс)', 'огноо уншигдсангүй', 'банкны шимтгэл'],
     'бүлэг: дүнгээр буурахаар эрэмбэлэгдэнэ (хамгийн том нь эхэнд)');
  eq([g[0].rows.length, g[0].sum], [2, 2000000], 'бүлэг: мөрийн тоо ба дүн нийлнэ');
  eq([g[2].rows.length, g[2].sum], [2, 400], 'бүлэг: жижиг дүн ч алдагдахгүй');
  eq(F.dropReasonGroups(rows).reduce((a, x) => a + x.rows.length, 0), rows.length,
     'бүлэг: НЭГ ч мөр алдагдахгүй');
  eq(F.dropReasonGroups([]), [], 'бүлэг: хоосон → хоосон');
  eq(F.dropReasonGroups([{ debit: 5 }])[0].why, '—', 'бүлэг: шалтгаангүй мөр «—» бүлэгт');
}
testDropGroups();
...
function testStatementBalances() {
  need(['statementMeta']);

  // ── ХААН: толгойн мөрөнд хоёр гарчиг ЗЭРЭГ, утга нь мөр бүрд ──
  const khan = [
    ['', '', '', '', '', '', 'Printed Date:', '2026-09-11'],
    ['', '', 'Депозит дансны дэлгэрэнгүй хуулга', '', '', '', '', ''],
    ['Хэрэглэгч:', '', '', 'ЧИМУН', '', '', 'Интервал: 2026-09-01-2026-09-11', ''],
    ['Валютын төрөл:', '', '', 'MNT', '', 'IBAN:', 'MN670005005222003015', ''],
    ['Гүйлгээний огноо', 'Салбар', 'Эхний үлдэгдэл', 'Кредит гүйлгээ', 'Дебит гүйлгээ', 'Эцсийн үлдэгдэл', 'Гүйлгээний утга', 'Харьцсан данс'],
    ['2026-09-01 09:39:50', '5022', 446581.81, 0.0, -30000.00, 416581.81, 'EB-Зарлага: машин', '5103006078'],
    ['2026-09-02 11:34:50', '5000', 416581.81, 800000.00, 0.0, 1216581.81, 'ДАНС ХООРОНД', '3675118079'],
    ['2026-09-11 19:46:24', '5022', 1216581.81, 0.0, -257777.00, 958804.81, 'EB-Зарлага мотор түлш', '5620056010'],
    ['', '', '', '', '', '', '', ''],
    ['Нийт дүн:', '', '', 800000.00, 287777.00, '', '', ''],
  ];
  const mk = F.statementMeta(khan);
  eq(mk.opening, 446581.81, 'ХААН: эхний үлдэгдэл = ЭХНИЙ мөрийн баганаас');
  eq(mk.closing, 958804.81, 'ХААН: эцсийн үлдэгдэл = СҮҮЛИЙН мөрийн баганаас');

  // ⛔ Гол регресс: баганын ГАРЧГИЙГ утга гэж уншиж 0 болгож байв
  ok(mk.opening !== 0 && mk.closing !== 0, 'ХААН: баганын гарчгаас 0 уншихаа болив');

  // ── Голомт: тус тусдаа шошготой мөр, дүн нь хажууд ──
  const golomt = [
    ['', '', '', '', '', 'Хуулганы огноо', '', '2026-09-11'],
    ['ДАНСНЫ ХУУЛГА', '', '', '', '', '', '', ''],
    ['', 'Дансны дугаар', '3675118079 [MNT]', 'IBAN', '', 'MN79 0015 0036 7511 8079', '', ''],
    ['', 'Харилцагчийн нэр', 'ЧИМУН', 'Гүйлгээний огноо', '', '2026-09-01 - 2026-09-11', '', ''],
    ['', 'Эхний үлдэгдэл', 141028, '', '', '', '', ''],
    ['', 'Гүйлгээний огноо', 'Гүйлгээний утга', 'Харьцсан дансны нэр', 'Харьцсан данс', 'Ханш', 'Орлого', 'Зарлага'],
    ['', '2026-09-01T09:32:57', 'POS', '', '', 1.0, '', 19000],
    ['', '2026-09-02T11:34:51', 'ДАНС ХООРОНД', 'ЧИМУН', '3635161180', 1.0, 541081, ''],
    ['', '', '', 'Нийт орлого', '', 541081, '', ''],
    ['', '', '', 'Нийт зарлага', '', 19000, '', ''],
    ['', '', '', 'Эцсийн үлдэгдэл', '', 663109, '', ''],
  ];
  const mg = F.statementMeta(golomt);
  eq(mg.acct, '3675118079', 'Голомт: дансны дугаар толгойноос ([MNT] хаягдана)');
  eq(mg.period, '2026-09-01 - 2026-09-11', 'Голомт: хугацаа толгойноос');
  eq(mg.opening, 141028, 'Голомт: эхний үлдэгдэл шошготой мөрөөс');
  eq(mg.closing, 663109, 'Голомт: эцсийн үлдэгдэл ФАЙЛЫН ДООД талаас');
  eq(141028 + 541081 - 19000, 663109, 'Голомт: эхний + орлого − зарлага = эцсийн');

  /* ⚠ Голомтын «Эхний үлдэгдэл» мөр нь ганцаараа байх ёстой — ХААН-ы баганын горим
     түүнийг булаахгүй (нэг мөрөнд хоёр гарчиг ЗЭРЭГ байхыг шаарддаг). */
  ok(mg.opening !== mg.closing, 'Голомт: баганын горим шошгын горимыг булаахгүй');

  // ── num(): цифргүй текст нь 0 БИШ ──
  const noDigit = [
    ['Эхний үлдэгдэл', 'Кредит гүйлгээ'],   // гарчиг л, дата мөр алга
  ];
  eq(F.statementMeta(noDigit).opening, null, 'цифргүй хөрш нүднээс 0 уншихгүй');
}
testStatementBalances();

// ═══ ХАСАГДСАН МӨРИЙН ШАЛТГААН + ДАНС (2026-09-11) ════════════════════════════
// «дотоод шилжүүлэг (өөрийн данс)» гэж 56 мөр · 38.8сая хасагдсаныг хэрэглэгч
// батлах аргагүй байв — аль данс болох нь хаана ч бичигдээгүй.
function testStmtDropWhy() {
  need(['stmtDropWhy']);
  const lbl = (d) => d === '5400145457' ? 'Голомт ••5457' : '';
  eq(F.stmtDropWhy({ why: 'дотоод шилжүүлэг (өөрийн данс)', acct: '5400145457' }, lbl),
     'дотоод шилжүүлэг (өөрийн данс) → Голомт ••5457', 'шалтгааны ард данс бичигдэнэ');
  eq(F.stmtDropWhy({ why: 'банкны шимтгэл' }, lbl), 'банкны шимтгэл', 'дансгүй мөр шалтгаагаа л хэлнэ');
  eq(F.stmtDropWhy({ why: 'огноо уншигдсангүй', acct: '' }, lbl), 'огноо уншигдсангүй', 'хоосон данс → нэмэлтгүй');
  eq(F.stmtDropWhy({ why: 'дотоод шилжүүлэг', acct: '9999999999' }, lbl), 'дотоод шилжүүлэг',
     'таних боломжгүй данс → шалтгаан дангаараа (хуурамч нэр зохиохгүй)');
  eq(F.stmtDropWhy({ why: 'х', acct: '  5400-145-457  ' }, (d) => d), 'х → 5400145457', 'зураас/зайтай данс цэвэрлэгдэнэ');
  eq(F.stmtDropWhy(null, lbl), '', 'хоосон мөр → хоосон');
  eq(F.stmtDropWhy({ acct: '5400145457' }, lbl), ' → Голомт ••5457', 'шалтгаангүй ч данс харагдана');

  // SCAN: хасагдсан мөр бүрд данс яваа эсэх — энэ нь шалгах ГАНЦ зам
  const at = src.indexOf("dropped.push({ date: r.date, memo: r.memo, debit: r.debit");
  ok(at > 0, 'scan: хасагдсан мөрийн бүртгэл олдов');
  const chunk = src.slice(at, at + 900);
  eq((chunk.match(/dropped\.push\(\{ date: r\.date[^}]*acct: r\.account/g) || []).length, 1,
     'scan: зарлагын хасалт (дотоод шилжүүлэг) дансаа авч явна');
  // Банкны шимтгэл 2026-09-11-нд хасалтаас ГАРСАН — зардал мөн (5700)
  ok(!/why: 'банкны шимтгэл'/.test(src),
     'scan: банкны шимтгэл хасагдахаа болив — зардал болж бүртгэгдэнэ');
}
testStmtDropWhy();
...
// болдог байв. Хаасан сар хөдөлөхгүй; алдаа гарвал дараагийн сард залруулга.
{
  ...
  // ── Хаахад бэлэн эсэх: дутуу хуулга · хаагдаагүй орлого · залгааны эвдрэл ──
  {
    ...
    /* ⭐ ХУВИЙН дансны зөрүү сар хаахыг БЛОКЛОХГҮЙ. `regAccts` = компанийн данс
       ...
       хязгаарлагдана. Өмнө нь «дутуу хуулга» шалгуур хувийн дансыг хасдаг байтал
       залгааны шалгуур хасдаггүй байсан — зөрчил. */
    const prsn = [stmts[0], { id: '99|2026-08-01|2026-08-31', acct: '9911223344', ccy: 'MNT',
  ...
  // ── SCAN: түгжээ бичих БҮХ гол замд тавигдсан хэвээр ──
  {
    ...
    // ⚠ Хуулгын импорт нь МӨРӨӨР шүүнэ (assertMonthOpen БИШ) — нэг хуулга 2 сарыг
    //   хамардаг нь хэвийн, бүхэлдээ татгалзвал хилээр дамжсан хуулга орж чадахгүй.
    gate('async function persistStatement', 'monthLocked');
...
// ═══ ЭЦСИЙН ҮЛДЭГДЭЛ ФАЙЛЫН ДООД ТАЛД (2026-09-11) ═══════════════════════════
// Толгойн 14 мөрийг л хайснаас болж оруулсан 3 хуулгын 2-т closing_stated ХООСОН
// болж, «эхний + орлого − зарлага = эцсийн» шалгуур ОГТ ажиллахгүй байв — хуулга
// бүтэн орсон эсэхийг шалгах гол хэмжүүр хоосон байсан.
{
  need(['statementMeta', 'stmtBalanceCheck']);
  const hdr = (extra) => [
    ['Дансны дугаар', '', '3635161180'],
    ['Гүйлгээний огноо', '', '2026-08-01 - 2026-08-31'],
    ['Эхний үлдэгдэл', '', '6739312'],
    ...(extra || []),
    ['Огноо', 'Гүйлгээний утга', 'Орлого', 'Зарлага', 'Үлдэгдэл'],
    ['2026-08-02', 'түрээс', '500000', '', '7239312'],
    ['2026-08-03', 'хүнс', '', '100000', '7139312'],
  ];

  // ⚠ УРТ файл (амьд хуулга 557 мөртэй): эцсийн үлдэгдэл нь толгойн 14 мөрөөс
  //   ХОЛ доор байдаг тул зөвхөн СҮҮЛийг хайж байж олно.
  const many = []; for (let i = 1; i <= 40; i++) many.push([`2026-08-${String(i % 28 + 1).padStart(2, '0')}`, 'гүйлгээ ' + i, '1000', '', '7239312']);
  const long1 = F.statementMeta([...hdr(), ...many, ['', '', '', 'Эцсийн үлдэгдэл', '7139312']]);
  eq(long1.closing, 7139312, 'хуулга: 40+ мөртэй файлын ХАМГИЙН ДООД эцсийн үлдэгдэл уншигдана');
  eq(long1.opening, 6739312, 'хуулга: урт файлд эхний үлдэгдэл ч хэвээр');
  eq(F.statementMeta([...hdr(), ...many]).closing, null, 'хуулга: эцсийн үлдэгдэлгүй урт файл → null (таамаглахгүй)');

  // ① Эцсийн үлдэгдэл ДООД талд (Голомтын бодит хэлбэр)
  const m1 = F.statementMeta([...hdr(), ['', '', '', 'Эцсийн үлдэгдэл', '7139312']]);
  eq(m1.opening, 6739312, 'хуулга: эхний үлдэгдэл толгойноос');
  eq(m1.closing, 7139312, 'хуулга: ЭЦСИЙН үлдэгдэл файлын ДООД талаас уншигдана');
  eq(m1.acct, '3635161180', 'хуулга: данс уншигдана');
  eq(m1.period, '2026-08-01 - 2026-08-31', 'хуулга: хугацаа уншигдана');

  // ② Толгойд байвал бас уншина (хуучин зан төлөв эвдрээгүй)
  eq(F.statementMeta(hdr([['Эцсийн үлдэгдэл', '', '7139312']])).closing, 7139312,
     'хуулга: толгойд байгаа эцсийн үлдэгдэл хэвээр уншигдана');

  // ③ Утга нь шошгоос ЗҮҮН талд (Голомт заримдаа ингэдэг)
  eq(F.statementMeta([...hdr(), ['7139312', 'Эцсийн үлдэгдэл']]).closing, 7139312,
     'хуулга: утга шошгоос ӨМНӨ байсан ч уншигдана');

  // ④ Гүйлгээний «Үлдэгдэл» БАГАНА эцсийн үлдэгдэл гэж андуурагдахгүй
  eq(F.statementMeta(hdr()).closing, null, 'хуулга: «Үлдэгдэл» багана шошго гэж андуурагдахгүй');

  // ⑤ Зөрүүтэй хуулга — тэнцэл УНАНА (мөр дутуу уншигдсаныг барина)
  const bad = F.statementMeta([...hdr(), ['', '', '', 'Эцсийн үлдэгдэл', '9999999']]);
  const chk = F.stmtBalanceCheck({ ccy: 'MNT', opening: bad.opening, closing_stated: bad.closing,
    closing_calc: bad.opening + 500000 - 100000 });
  eq(chk.ok, false, 'хуулга: эцсийн үлдэгдэл зөрвөл тэнцэл УНАНА');
  eq(chk.diff, 7139312 - 9999999, 'хуулга: зөрүүний дүн гарна');

  // ⑥ Толгой ба сүүл давхцах жижиг файлд 2 удаа уншиж эвдрэхгүй
  eq(F.statementMeta([['Эхний үлдэгдэл', '100'], ['Эцсийн үлдэгдэл', '200']]).closing, 200,
     'хуулга: жижиг файлд ч ажиллана');

  // SCAN — сүүл хайх нь хэвээр
  ok(/rows\.slice\(Math\.max\(14, rows\.length - 20\)\)/.test(src),
     'scan: файлын СҮҮЛийг ч хайдаг хэвээр (эцсийн үлдэгдэл доор байдаг)');
}

// ═══ ШИЛЖИЛТИЙН ХИЛ: хуулга МӨРӨӨР алгасна (2026-09-11) ══════════════════════
// Нэг хуулга 2 сарыг хамардаг нь хэвийн (жиш. 08-01…09-11). Хаасан сар таарвал
// файлыг бүхэлдээ татгалзвал хилээр дамжсан хуулга ОРОХ БОЛОМЖГҮЙ болно.
{
  const at = src.indexOf('async function persistStatement');
  const body = src.slice(at, at + 1600);
  ok(!/assertMonthOpen/.test(body),
     'хил: хуулгыг бүхэлдээ татгалзахаа больсон (assertMonthOpen хасагдсан)');
  ok(/filter\(x => !monthLocked\(String\(x\.dt/.test(body),
     'хил: хаасан сарын ОРЛОГЫН МӨР л алгасагдана');
  ok(/saveStatementImport\(built\.stmt, fresh\)/.test(body),
...
// гарын үсгийн систем ЧИМЭЭГҮЙ нэг гарын үсэг болж буурна.
{
  ...
  // ⛔ Батлах хүлээж буй нь ЭХЭНД. Тоолох жагсаалт 40 мөр гаргадаг тул доор
  //    тавьвал батлах ажил дэлгэцийн гадна үлдэж «алга болно» (2026-09-14-нд
  //    амьд системд яг ингэж болсон: толгойд «19 батлах хүлээж буй» гэж
  //    бичигдсэн хэрнээ жагсаалт нь олдохгүй байв).
  const b = src.indexOf('function openingBlockHtml(');
  const body = src.slice(b, src.indexOf('function renderStockCount(', b));
  const iWait = body.indexOf('waitList +'), iCount = body.indexOf('countList\n');
  ok(iWait > 0, 'scan: батлах жагсаалт эхэнд угсрагдана');
  ok(iWait < body.indexOf('+ countList'),
     '⛔ scan: батлах хүлээж буй нь тоолох жагсаалтаас ӨМНӨ гарна');
  // ⛔ Баталгаажсан бараа ХАРАГДАХ ЁСТОЙ. Хоёр гарын үсгийн бүх утга нь
  //    хариуцлага мөрдөгдөх явдал — харагдахгүй гарын үсэг нь гарын үсэг биш.
  ok(/sign === 'done'/.test(body), 'scan: баталгаажсан барааны жагсаалт бүрдүүлэгддэг');
...
// GitHub Issue-д үлддэг байв. `tools/night_notify.py` өглөө утсанд хэлнэ.
{
  ...
  // ⛔ ХОЁР ЗАМ ЗЭРЭГ (2026-09-24). Өмнө нь «алдаа байвал зөвхөн түүнийг зас»
  //    байсан тул ЗАСАГДАХГҮЙ нэг алдаа (#565 — Facebook-ийн in-app браузер)
  //    4 шөнө дараалан бүх автоматыг зогсоосон: өр ч хийгдээгүй.
  ok(/lane:\s*\[error,\s*debt\]/.test(wf), 'шөнө: алдаа ба өр ХОЁУЛАА зэрэг явна');
  ok(/fail-fast:\s*false/.test(wf), 'шөнө: нэг зам унахад нөгөө нь цуцлагдахгүй');

  // ⛔ Алдааг ХАМГИЙН ХУУЧНААС ав. Хамгийн ШИНЭЭС авбал шинэ засагдахгүй
  //    алдаа гарах бүрд жижүүр дахин тэр дээрээ гацна.
  ok(/sort:created-asc/.test(wf), 'шөнө: алдаа хамгийн хуучнаас сонгогдоно');
  ok(!/--label "алдаа" --state open --limit 1/.test(wf),
     'шөнө: хамгийн шинэ алдааг авах хуучин сонголт буцаж ирээгүй');

  // ⛔ Хоёр төрөл сонголтоос хасагдана — эс бөгөөс гацаа давтагдана.
  ok(/-label:"засагдахгүй"/.test(wf), 'шөнө: засаж чадаагүй алдаа сонголтоос хасагдана');
  ok(/-label:"сайт"/.test(wf), 'шөнө: mevent.mn (ӨӨР репо) сонголтоос хасагдана');

  // ⛔ «Боломжгүй» гэж дуусахад ШОШГО заавал тавигдана. Эс бөгөөс маргааш
  //    ижил алдааг дахин авч дахин «боломжгүй» гэж дуусна (4 шөнө ингэж үрэгдсэн).
  ok(/--add-label "засагдахгүй"/.test(wf), 'шөнө: засаж чадаагүй алдаа шошгологдоно');

  // ⛔ Салбарын нэрэнд зам орно — хоёр зам нэг минутад push хийвэл мөргөлдөнө.
  ok(/BR="claude\/night-\$MODE-/.test(wf), 'шөнө: салбарын нэр зам бүрд өөр');

  // ⛔ Мэдэгдэл СҮҮЛИЙН мөрийг ганцаар авч болохгүй — шөнө 2 мөр бичигдэнэ.
  ok(/merge_summaries\(/.test(py), 'шөнө: мэдэгдэл нэг шөнийн бүх мөрийг нэгтгэнэ');
  ok(!/return parse_summary\(cs\[-1\]/.test(py), 'шөнө: сүүлийн мөрийг ганцаар авахгүй');
...
// ═══ Элэгдлийн нас: машин, генератор 5 жил БИШ (2026-09-24) ═══
// Өгөгдмөл 5 жилийн хайрцагт МАШИН (HINO 68 сая₮) ба ГЕНЕРАТОР орж байв —
// бүх зүйл 5 жилээр элэгдвэл машины элэгдэл хоёр дахин их гарна.
{
  const { deprecYearsFor } = F;
  eq(deprecYearsFor('HINO Ranger (крантай) — 9466 УКН').years, 10, 'нас: ачааны машин 10 жил');
  eq(deprecYearsFor('Hyundai Porter — 1840 УНД').years, 10, 'нас: Porter 10 жил');
  eq(deprecYearsFor('Nissan Vanette — 0257 УАР').years, 10, 'нас: Vanette 10 жил');
  // ⛔ «Цасны машин» нь «машин» гэсэн үг агуулдаг ч ТЕХНИК — тээврийн дүрэм
  //   брэндээр таньдаг тул андуурахгүй.
  eq(deprecYearsFor('Хиймэл цасны машин 1500Вт').years, 4, 'нас: цасны машин техник 4 жил (тээвэр БИШ)');
  eq(deprecYearsFor('Утааны машин (3000Вт)').years, 4, 'нас: утааны машин 4 жил');
  eq(deprecYearsFor('Генератор 32квт (герман)').years, 8, 'нас: генератор 8 жил');
  eq(deprecYearsFor('Цэнэг хураагуур 3800Вт·ц').years, 8, 'нас: цэнэг хураагуур 8 жил');
  eq(deprecYearsFor('Тавиур (агуулах)').years, 10, 'нас: агуулахын тавиур 10 жил');
  eq(deprecYearsFor('Хурдан хөлдөөгч шкаф (9 тавиур)').years, 8, 'нас: гал тогооны тоног 8 жил (тавиур БИШ)');
  eq(deprecYearsFor('Дрон Mavic 4 Pro').years, 3, 'нас: дрон 3 жил');
  eq(deprecYearsFor('Хийлдэг гудас 1 хүний').years, 3, 'нас: хийлдэг 3 жил');
  eq(deprecYearsFor('K3 чанга яригч').years, 6, 'нас: чанга яригч 6 жил');
  eq(deprecYearsFor('KS28 сабвүфер (18")').years, 6, 'нас: сабвүфер 6 жил');
  eq(deprecYearsFor('Мэргэжилийн хөгжим array-line').years, 6, 'нас: мэргэжлийн хөгжим 6 жил (техник БИШ)');
  // Хуучин дүрмүүд эвдрээгүй эсэх
  eq(deprecYearsFor('Асар 10м:20м').years, 8, 'нас: асар 8 жил хэвээр');
  eq(deprecYearsFor('Ширээний бүтээлэг 182×76 Цагаан').years, 2, 'нас: бүтээлэг 2 жил хэвээр');
  eq(deprecYearsFor('Тайзны электрон оч шүршигч').years, 4, 'нас: оч шүршигч 4 жил хэвээр');
