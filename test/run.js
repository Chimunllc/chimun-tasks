...
{
  eq(F.asarSizeLabel('M-311', 5), '18×25м', 'асар: 5 модуль = 18×25м');
  eq(F.asarSizeLabel('M-311', 8), '18×40м', 'асар: 8 модуль = 18×40м (A иж бүрдлийн дээд)');
  eq(F.asarSizeLabel('M-309', 5), '12×25м', 'асар: 12м өргөн 5 модуль');
  eq(F.asarSizeLabel('M-309', 1), '12×5м', 'асар: 1 модуль');
  eq(F.asarSizeLabel('M-001', 3), '', 'асар: модуль биш бараанд хэмжээ гарахгүй');
  eq(F.asarSizeLabel('M-311', 0), '18×5м', 'асар: 0 → доод хязгаар 1 модуль');
  ...
  eq(F.asarLengthOptions('M-309'), [5, 10, 15, 20, 25], 'асар: 12м A → 25м хүртэл');
  ...
  const p12 = MODP.find(x => x.sku === 'M-309');
  const p18 = MODP.find(x => x.sku === 'M-311');
  eq(p12.price * 5, 5000000, 'үнэ: 12×25 = 5 сая (хуучинтай ижил)');
  eq(p18.price * 5, 9900000, 'үнэ: 18×25 = 9.9 сая (хуучинтай ижил)');
  eq(p18.price * 8, 15840000, 'үнэ: 18×40 = 15.84 сая (хуучинтай ижил)');
  eq(p12.stock, 5, 'нөөц: 12м A = 5 модуль');
  eq(MODP.find(x => x.sku === 'M-311').stock, 8, 'нөөц: 18м A = 8 модуль');
