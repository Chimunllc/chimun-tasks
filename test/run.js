...
// Тиймээс татагч ХҮН авсныг (`Callee Answer Second > 0`) л `answered` гэж бичнэ.
{
  ...
  // ⚠ «miss» сануулга нь 2026-09-17-ээс ДЭЛГЭРЭНГҮЙ ЛОГООС бодогдоно (цагийн
  //   нийлбэрт дуудлагын урт байхгүй тул богино тасалсныг «алдсан» гэж
  //   тоолж, хариулалт худлаа бага харагддаг байв). Тиймээс ажлын цагийн
  //   алдсан дуудлагыг логоор өгнө.
  const missLog = Array.from({ length: 25 }, (_, i) => ({
    direction: 'in', peer: '9911223' + (i % 10), answer_sec: 0, call_sec: 40,
    started_at: `2026-09-1${i % 5}T03:00:00+00:00`,     // UB 11:00 — ажлын цаг
  }));
  const tips = F.callAdvice(p, 9, 18, offLog.concat(missLog));
  ok(tips.some(t => t.kind === 'miss'), 'дуудлага: ажлын цагийн алдагдлыг сануулна');
  // ⛔ Богино тасалсан нь «алдсан» гэж тоологдохгүй — 40 ширхэг 6 секундын
  //    дуудлага сануулга ҮҮСГЭХГҮЙ (тэдгээр лид биш).
  ok(!F.callAdvice(p, 9, 18, kfcLog.map(c => Object.assign({}, c,
       { peer: '99112233', started_at: '2026-09-10T03:00:00+00:00' })))
     .some(t => t.kind === 'miss'), 'дуудлага: богино тасалсан сануулга үүсгэхгүй');
...
// ── ХАРИУЛАЛТЫН ХУВЬ = ДЭЛГЭРЭНГҮЙ ЛОГООС (2026-09-17) ─────────────────────
// `pbx_calls_hourly`-д дуудлагын УРТ байхгүй тул 13 секундээс богино тасалсныг
// «алдсан» гэж тоолж, хариулалт 54% гэж ХУДЛАА бага харагддаг байв (бодит нь
// 77%). Алдсан дуудлагын дэлгэц зөв тоолдог байсан тул хоёр дэлгэц зөрж байв.
{
  const mk = (h, ans, cs, dir) => ({
    direction: dir || 'in', peer: '99112233',
    started_at: `2026-09-10T${String(h - 8).padStart(2, '0')}:00:00+00:00`,  // UB цаг
    answer_sec: ans, call_sec: cs,
  });
  const log = [
    mk(10, 30, 45),   // ажлын цаг, ярьсан
    mk(11, 0, 40),    // ажлын цаг, 40 сек хүлээгээд алдсан
    mk(12, 0, 5),     // ⛔ 5 сек — лид БИШ, тоологдохгүй
    mk(21, 0, 60),    // ажлын цагийн ГАДНА — тоологдохгүй
    mk(13, 20, 30, 'out'),  // гарсан дуудлага — тоологдохгүй
    Object.assign(mk(14, 0, 30), { peer: '101' }),  // дотоод дугаар — хүн биш
  ];
  const r = F.pbxAnswerRate(log, { ws: 9, we: 18 });
  eq(r.calls, 2, 'хариулалт: зөвхөн жинхэнэ оролдлого');
  eq(r.answered, 1, 'хариулалт: авсан нь');
  eq(r.missed, 1, 'хариулалт: алдсан нь');
  eq(r.rate, 50, 'хариулалт: хувь');
  eq(F.pbxAnswerRate([], {}).rate, 0, 'хариулалт: хоосон → 0');
  eq(F.pbxAnswerRate(null, {}).calls, 0, 'хариулалт: null → унахгүй');
  // ⚠ Цаг уншигдаагүй мөрийг ХАСАХГҮЙ — форматын өөрчлөлтөд бүгд алга болохоос.
  eq(F.pbxAnswerRate([{ direction: 'in', peer: '99112233', started_at: 'муу',
                        answer_sec: 5, call_sec: 20 }], {}).calls, 1,
     'хариулалт: цаг уншигдаагүй мөр үлдэнэ');

  const asrcR = fs.readFileSync(path.join(__dirname, '..', 'app.js'), 'utf8');
  // ⛔ Дэлгэц цагийн нийлбэрийн `bizRate` рүү БУЦАХГҮЙ — тэр нь 13 секундын
  //    шүүлтгүй тул хариулалтыг худлаа бага харуулна.
  ok(/ansR\.rate/.test(asrcR) && !/pbx\.bizRate/.test(asrcR),
     'scan: дэлгэц зөв эх сурвалжаас хариулалт харуулна');
  ok(/pbxAnswerRate\(state\.pbxLog/.test(asrcR), 'scan: дэлгэрэнгүй логоос бодогдоно');
  // ⛔ ЗӨВЛӨГӨӨ ба KPI ч ижил эх сурвалжаас — гурван газар өөр тоо гаргавал
  //    хүн аль нь үнэн болохыг мэдэхгүй.
  ok(!/p\.bizRate < 70/.test(asrcR) && /real\.rate < 70/.test(asrcR),
     'scan: зөвлөгөө жинхэнэ дуудлагаар шалгана');
  ok(!/\$\{pbx\.calls \? pbx\.rate/.test(asrcR), 'scan: KPI түүхий хувь харуулахгүй');
}
