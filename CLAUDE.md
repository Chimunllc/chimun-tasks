...
- **Захиалгын түүх — үндсэн самбарт нэгтгэсэн (2026-06-23):** Хэрэглэгчийн хүсэлтээр Booqable-г UI-аас БҮР МӨСӨН халж, бүх 1354 түүх захиалгыг үндсэн **Захиалга** view-д аппын ТӨРӨЛХ карт маягаар оруулав. `renderOrders`-д эх сурвалж toggle: **"📋 Идэвхтэй"** (M-Event ажлын самбар, хэвээр) ⇄ **"📚 Бүх захиалга (N)"** (`renderBooqableArchive`). Архив нь аппын `order-card` CSS ашиглана (M-Event-тэй ЯГ адил). `bqMapStatus`: Booqable статус→аппын статус (archived/stopped→Дууссан, reserved/started→Идэвхтэй, canceled→Цуцалсан, draft→Ноорог), badge `orderStatusClass`-аар. Статус филтер (аппын статусаар) + хайлт(нэр/утас/имэйл/дугаар) + нийт дүн + бараа задлах (native хүснэгт). `loadBooqableOrders`/`loadBooqableOrderItems` → `state.bqOrders`/`bqOrderItems` (top-level, lazy). `attachBooqableArchiveHandlers`. **`bq_09_orders.sql`** (bq_v_orders + bq_v_order_items) + **`bq_10_contact.sql`** (харилцагчийн утас 1057/имэйл 364/хаяг + захиалгын хүргэх хаяг 17 backfill, bq_v_orders-д нэмэв — `properties.phone`/`main_address`/`delivery_address`-ээс). Booqable нэр UI-д АЛГА (зөвхөн дотоод/код/постгресд).
