...
// нэг өдөр хоёр газар зэрэг зарж болох давхар захиалгын нүх үүсдэг.
{
  ...
  eq(F.asarLengthOptions('M-316'), [10, 15, 20], 'асар: 10м өргөн → 10м-ээс 20м хүртэл');
  ...
  eq(F.asarSizeLabel('M-316', 4), '10×20м', 'асар: 10м өргөн 4 модуль = 10×20м');
  ...
  eq(LEG.length, 20, 'цэгцлэлт: 20 хуучин уртын бүртгэл (10×20 нэмэгдэв)');
}
...
// Тиймээс архивлахын өмнө толинд (product_aliases) шинэ модуль бараа руу заана.
{
  ...
  eq(MAP['M-001'], 'M-316', 'цэгцлэлт: 10×20 → 10м өргөн модуль');
  ...
  MODP.forEach(m => {
    ...
    // ⚠ M-316 (10м) — худалдан авалтын өртөг ₮-өөр бүртгэгдээгүй (гэрээнд ¥35,000).
    // Таамаглаж бичихгүй: хэрэглэгч бодит дүнг хэлэхэд оруулна.
    if (m.sku !== 'M-316') ok(m.cost > 0, `${m.sku}: нэг модулийн өртөг`);
...
// (б) хуучин захиалгын мөр тулгагдсаар үлдэхийн тулд толины зураглалтай байх.
{
  ...
  const rows = [
    ...
    { sku: 'M-001', name: 'Асар 10м өргөн 20м' },  // жагсаалтад алга → хөндөхгүй
  ...
  eq(plan.skip.map(p => p.sku), ['M-296', 'M-001', 'M-315'], 'хасалт: зураглалгүй ба жагсаалтын гадна бүгд алгасагдана');
